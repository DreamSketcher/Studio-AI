...
import ai_studio_core.env_core.torch_setup as ts
...
class TestPickTorchVariant:
    def test_pref_cpu(self, monkeypatch):
        monkeypatch.setattr(
            "ai_studio_core.settings_store.load_settings", lambda: {"torch_device_preference": "cpu"}
        )
    ...
    def test_pref_gpu_not_broken(self, monkeypatch):
        monkeypatch.setattr(
            "ai_studio_core.settings_store.load_settings", lambda: {"torch_device_preference": "gpu"}
        )
    ...
    def test_nvidia_cuda_ok(self, monkeypatch):
        monkeypatch.setattr("ai_studio_core.settings_store.load_settings", lambda: {})
        monkeypatch.setattr(ts, "get_broken_torch_variants", lambda: set())
        gpu_info = {"vendor": "nvidia", "cuda_version": "12.8"}
        variant, url = ts._pick_torch_variant(gpu_info)
        assert variant == "cu128"
