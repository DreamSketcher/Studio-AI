...
def get_python_env_info() -> dict:
    from ai_studio_core.env_core.llama_setup import get_site_packages
...
def _resolve_recovery_torch_variant() -> tuple:
    ...
    try:
        from ai_studio_core.env_core import torch_setup
...
def _torch_already_ok(variant: str) -> bool:
    ...
    """
    try:
        from ai_studio_core.env_core.torch_setup import torch_status
...
def _get_av_pin() -> str:
    ...
    try:
        ...
        pass
    try:
        from ai_studio_core.env_core.rvc_setup import AV_PIN
...
def run_error_recovery(progress_cb=None) -> list:
    ...
    for key, pkg in sorted(matched_specs.items()):
        ...
        # ai_studio_core.env_core.rvc_setup.install_rvc(), который делает всё это
        ...
        if key == "rvc_python":
            ...
            try:
                from ai_studio_core.env_core import rvc_setup
