...
import subprocess
import shutil
from ai_studio_core.paths import BASE_DIR
...
def detect_torch_build(site_packages: str):
    ...
    ЕДИНУЮ логику выбора сборки из ai_studio_core.env_core.torch_setup, чтобы RVC-путь
    ...
    """
    from ai_studio_core.env_core.torch_setup import _TORCH_INDEX_URLS
    ...
    try:
        from ai_studio_core.env_core.torch_setup import _pick_torch_variant
        from ai_studio_core.env_core.cpu_gpu import detect_gpu
...
def _run_pip_capture(cmd, env, progress_cb=None):
    ...
    from ai_studio_core.env_core.diagnostics import _read_pip_output
...
def install_rvc(progress_cb=None) -> dict:
    ...
    from ai_studio_core.env_core.install_state import _can_clear_diagnostics_cache
    ...
    if extra_deps:
        ...
        # ЕДИНУЮ логику ai_studio_core.env_core.torch_setup — тот же механизм, что у
