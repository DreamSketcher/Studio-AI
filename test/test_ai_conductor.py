...
from ai_studio_core.ai_conductor import _fallback_params, _validate_map, _word_count
...
class TestConductIntegrationWithMock:
    ...
    def test_conduct_returns_fallback_on_invalid_json(self, monkeypatch):
        ...
        # подменяем ai_studio_core.gpt_client
        monkeypatch.setitem(sys.modules, "ai_studio_core.gpt_client", fake_gpt)
        # также для относительного импорта ai_studio_core.ai_conductor -> .gpt_client
        # Нужно подсунуть модуль как ai_studio_core.gpt_client уже есть, from .gpt_client попробует относительный,
        # но он найдёт через sys.modules

        from ai_studio_core.ai_conductor import conduct
